...
## D-078 — La 012 acierta 14 de 15, y el fallo que parecía del modelo era una tarjeta (2026-09-18)
...
estaba pendiente), crear un cobro con día fijo, un recurrente al que le falta el
día y un aporte sin monto. Como en D-074, lo que se mide no es el texto sino la
**elección de herramienta**: que no escriba donde debe preguntar y que sí proponga
lo que el usuario pide.

**Resultado: 14 de 15.** Elige bien en las catorce, incluidas las que importan:
...
**El fallo que parecía del modelo no era del modelo.** «Register la suscripción
de 40 mil cada mes» → el asistente preguntó «¿Qué día del mes se cobra?» y no
escribió nada: cero filas en `assistant_writes`. Lo que el oráculo marcaba como
«propuso escribir donde no debía» era un bug de render. El resultado
`falta-dia` de `proponerRecurrente` (un recurrente «cada mes» sin día) no estaba
contemplado en `TarjetaDeAccion`, que solo manejaba el `falta-fecha` de los
movimientos: se dibujaba una tarjeta de confirmación **vacía** con botones sin
propuesta detrás. Es la misma clase que D-068 y D-074 —falta la segunda mitad de
una herramienta, la que la enseña—, pero la sacó la medición, no la vista.
Corregido: `falta-dia` se muestra como la pregunta que es, igual que
`falta-fecha`.

**Y una variación de presentación.** «¿Cómo voy con los presupuestos?» responde
en tres de cuatro pasadas el avance en porcentaje («has gastado el sesenta por
ciento del tope») y no repite la cifra del tope. No es elección de herramienta:
es que el número pedido no siempre aparece. Acertó con la cifra una de cuatro.
...
**Qué cambió y qué no.** Del modelo no cambió nada: la 012 ya estaba construida
y esta es su primera medida con modelo real (D-057 solo cubrió las utilidades de
la 003). La puerta cumplió: ni un peso se escribió sin confirmar. Lo que cambió
fue la interfaz, para enseñar la pregunta de `falta-dia` como pregunta.
